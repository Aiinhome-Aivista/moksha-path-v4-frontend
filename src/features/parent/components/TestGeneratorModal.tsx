...
const TestGeneratorModal: React.FC<TestGeneratorModalProps> = ({
  ...
  const handleChildToggle = (childId: number) => {
    const isSelected = form.student_ids.includes(childId);
    const newSelection = isSelected
      ? form.student_ids.filter(id => id !== childId)
      : [...form.student_ids, childId];
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  {children.length > 0 && (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        if (form.student_ids.length === children.length) {
                          setForm(p => ({ ...p, student_ids: [] }));
                        } else {
                          const allIds = children.map(c => c.student_id);
                          setForm(p => ({ ...p, student_ids: allIds }));
                        }
                      }}
                      className="text-xs text-[#b0cb1f] hover:underline font-semibold"
                    >
                      {form.student_ids.length === children.length ? "Deselect All" : "Select All"}
                    </button>
                  )}
                </div>
                <div className={`border border-gray-300 rounded-md p-3 max-h-48 overflow-y-auto ${children.length === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                  {children.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {children.map((child) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={form.student_ids.includes(child.student_id)}
                            onChange={() => handleChildToggle(child.student_id)}
                            className="mt-0.5 h-4 w-4 text-[#b0cb1f] border-gray-300 rounded focus:ring-[#b0cb1f]"
                          />
                          <span className="text-sm text-gray-700 flex-1">{child.full_name}</span>
                        </label>
                      ))}
                    </div>
                  )}
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {form.student_ids.length} child(ren) selected
                </p>
              </div>
