...
const Tests: React.FC<TestsProps> = ({
  ...
  const [parentAssessments, setParentAssessments] = useState<any[]>([]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [parentAssessments.length, subjectName, chapterIds, statusFilter]);
  ...
  // Fetch parent assessments
  const fetchParentAssessments = async () => {
    try {
      setIsLoading(true);
      // Re-using the teacher student status dashboard API as it might be returning similar data or 
      // there should be a dedicated parent endpoint, using teacher_student_status_dashboard for now
      // assuming it gets the assessments assigned by parent if token is parent's
      const response = await ApiServices.getTeacherStudentStatusDashboard();
      if (response.data?.status === "success") {
        setParentAssessments(response.data.data || []);
      }
    } catch (error: any) {
      showToast(
  ...
  const handleGenerate = async (data: GeneratorData) => {
    try {
      ...
      let response = await ApiServices.assignAssessmentToStudents(payload);

      if (response.data?.status === "success") {
        showToast(response.data.message || "Assessment created successfully!", "success");
        setSummaryStatus("success");
        setSummaryMessage("The assessment has been successfully assigned to your child(ren).");
        setSummaryData(data);
        closeGenerator();
        openSummary();
        fetchParentAssessments();
  ...
  const getChapterNames = (ids: number[]) => {
    ...
    const names = ids.map((id) => {
      ...
      return chapter ? chapter.name : id;
    });
    return names.join(", ");
  };
  ...
  const filteredAssessments = parentAssessments.filter((assessment) => {
    ...
    if (searchQuery) {
      ...
      const matchSubject = assessment.subject_name?.toLowerCase().includes(query);
      const matchTestName = assessment.test_name?.toLowerCase().includes(query);
      const matchChapters = assessment.chapter_ids ? getChapterNames(assessment.chapter_ids).toLowerCase().includes(query) : false;
      const matchTopics = assessment.topic_ids ? getTopicNames(assessment.topic_ids).toLowerCase().includes(query) : false;
    ...
    if (statusFilter !== "All") {
      ...
      if (statusFilter === "Expired") {
        if (isCompleted || !expired) return false;
      } else if (statusFilter === "Pending") {
        if (!isPending || expired) return false;
          ...
          <div>
            <div className="flex flex-col xl:flex-row justify-between items-center gap-4 mb-4">
              <div className="relative w-full lg:w-[400px]">
                ...
                <input
                  type="text"
                  className="block w-full pl-11 pr-4 py-2 border border-gray-300 rounded-full leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#b0cb1f] focus:border-[#b0cb1f] sm:text-sm transition duration-150 ease-in-out shadow-sm"
            ...
            {filteredAssessments.length > 0 ? (
              <div className="mb-8">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Assigned to Children</h3>
                <div className="space-y-4">
                  {filteredAssessments
                    ...
                    .map((assessment) => (
                      <div
                        key={assessment.set_id || Math.random()}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-bold text-gray-700">
                                {new Date(assessment.assigned_date || new Date()).toLocaleString("en-IN", {
                                  month: "short",
                                  day: "numeric",
                                  year: "numeric",
                                  hour: "numeric",
                ...
                {parentAssessments.length > ITEMS_PER_PAGE && (
